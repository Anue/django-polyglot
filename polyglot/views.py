...
def set_language_replace(request, cut_next=False):
    """Redirect to a given url while setting the chosen language in
    the session or cookie. The url and the language code need to be
...
def redirect_to_i18n(request, url, **kwargs):
    ...
    The given url may contain dict-style string formatting, which will
    be used against the params in the URL. For example, to redirect
    from ``/foo/<lang>/`` to ``/bar/<lang>/``, you could use the
    following URLconf:
